...
func subTestCreateInvalidAllocation(t *testing.T, turnSocket net.PacketConn) {
	m, _ := newTestManager()
	if a, err := m.CreateAllocation(nil, turnSocket, 0, proto.DefaultLifetime, nil); a != nil || err == nil {
		t.Errorf("Illegally created allocation with nil FiveTuple")
	}
	if a, err := m.CreateAllocation(randomFiveTuple(), nil, 0, proto.DefaultLifetime, nil); a != nil || err == nil {
		t.Errorf("Illegally created allocation with nil turnSocket")
	}
	if a, err := m.CreateAllocation(randomFiveTuple(), turnSocket, 0, 0, nil); a != nil || err == nil {
...
func subTestCreateAllocation(t *testing.T, turnSocket net.PacketConn) {
	...
	if a, err := m.CreateAllocation(fiveTuple, turnSocket, 0, proto.DefaultLifetime, nil); a == nil || err != nil {
...
func subTestAllocationTimeout(t *testing.T, turnSocket net.PacketConn) {
	...
	for index := range allocations {
		...
		a, err := m.CreateAllocation(fiveTuple, turnSocket, 0, lifetime, nil)
		if err != nil {
...
func subTestManagerClose(t *testing.T, turnSocket net.PacketConn) {
	...
	a1, _ := m.CreateAllocation(randomFiveTuple(), turnSocket, 0, time.Second, nil)
	allocations[0] = a1
	a2, _ := m.CreateAllocation(randomFiveTuple(), turnSocket, 0, time.Minute, nil)
	allocations[1] = a2
